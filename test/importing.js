...
test('importing: ignore dirs option turned off', function (t) {
  DWeb(fixtures, { temp: true }, function (err, dweb) {
    ...
    dweb.importFiles({ ignoreDirs: false }, function () {
      var stream = dweb.archive.history()
      var hasFolder = false
      var hasRoot = false
...
test('importing: import with opts.useDatIgnore false', function (t) {
  ...
  DWeb(fixtures, { temp: true }, function (err, dweb) {
    ...
    var importer = dweb.importFiles({ useDatIgnore: false }, function (err) {
      t.error(err)
...
test('importing: import from hidden folder src', function (t) {
  tmpDir(function (_, dir, cleanup) {
    ...
    DWeb(dir, { temp: true }, function (err, dweb) {
      ...
      dweb.importFiles(function (err) {
        t.error(err)
        t.same(dweb.archive.version, 1, 'archive has 1 file')
        dweb.archive.stat('/hello.txt', function (err, stat) {
          t.error(err, 'no error')
...
test('importing: make sure importing .. fails', function (t) {
  tmpDir(function (_, dir, cleanup) {
    ...
    DWeb(dir, { temp: true }, function (err, dweb) {
      ...
      dweb.importFiles(function (err) {
        t.error(err)
        dweb.archive.readdir('/', function (err, list) {
          t.error(err, 'no error')
